...
const AdminCompliance = () => {
  ...
  const { data: companyDocs } = useQuery({
    queryKey: ["admin-company-docs"],
    queryFn: async () => {
      const { data, error } = await supabase.from("company_documents").select("company_id, doc_type, status, expiry_date").limit(500);
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const expiringFields = (companies || []).reduce((acc, c) => {
    const fields = [
    ...
    for (const f of fields) {
      if (f.date) {
        const days = differenceInDays(parseISO(f.date), today);
    ...
    <AdminLayout>
      ...
      <div className="grid sm:grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl border border-[hsl(220,15%,13%)] bg-[hsl(220,18%,10%)] p-5">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-[hsl(220,10%,40%)]">Total Companies</p>
          <p className="text-2xl font-bold text-white mt-1">{companies?.length || 0}</p>
        </div>
      ...
      {(expiringFields.expired.length > 0 || expiringFields.expiring.length > 0) && (
        <div className="rounded-xl border border-red-500/20 bg-[hsl(220,18%,10%)] p-5 mb-6">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {expiringFields.expired.map((e, i) => (
            ...
            {expiringFields.expiring.map((e, i) => (
      ...
      {isLoading ? <Skeleton className="h-64 w-full bg-[hsl(220,15%,15%)]" /> : (
        <div className="rounded-xl border border-[hsl(220,15%,13%)] bg-[hsl(220,18%,10%)] overflow-hidden">
          <div className="px-4 py-3 border-b border-[hsl(220,15%,13%)]">
            <h2 className="text-sm font-semibold text-white">Company Compliance Status</h2>
          </div>
...
function parseISO(dateStr: string) { return new Date(dateStr); }
function differenceInDays(date1: Date, date2: Date) { return Math.floor((date1.getTime() - date2.getTime()) / 86400000); }
