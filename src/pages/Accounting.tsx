...
import { TrendingUp, TrendingDown, DollarSign, BarChart3, Search, Receipt } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/contexts/AuthContext";
import { useQuery } from "@tanstack/react-query";
import { useNavigate } from "react-router-dom";
import { SOADashboard } from "@/components/shipment/SOADashboard";
    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Financial overview, P&L, and statement of account</p>
        </div>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="w-full justify-start h-auto gap-1 bg-transparent p-0 border-b border-border rounded-none pb-0 mb-6">
          <TabsTrigger value="pnl" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-accent data-[state=active]:text-accent rounded-none px-4 pb-3 text-sm gap-1.5">
            <BarChart3 className="h-3.5 w-3.5" /> Profit & Loss
          </TabsTrigger>
          <TabsTrigger value="soa" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-accent data-[state=active]:text-accent rounded-none px-4 pb-3 text-sm gap-1.5">
            <Receipt className="h-3.5 w-3.5" /> Statement of Account
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="pnl" className="mt-0">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <MetricCard label="Total Revenue" value={fmt(totals.revenue)} icon={<TrendingUp className="h-4 w-4 text-emerald-600" />} />
            <MetricCard label="Total COGS" value={fmt(totals.cost)} icon={<TrendingDown className="h-4 w-4 text-destructive" />} />
          ...
          <Card>
            <CardContent className="p-0">
              {shipmentsLoading ? (
                ...
                <div className="p-12 text-center text-muted-foreground text-sm">No shipments found.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.map((s) => (
                        <tr key={s.id} className="border-b last:border-0 hover:bg-secondary/50 cursor-pointer transition-colors" onClick={() => navigate(`/dashboard/shipments/${s.id}`)}>
                          ...
                          <td className={`p-4 text-right font-mono font-medium ${s.grossProfit >= 0 ? "text-emerald-600" : "text-destructive"}`}>{fmt(s.grossProfit)}</td>
                          <td className="p-4 text-right font-mono text-muted-foreground">{fmt(s.totalExpense)}</td>
                          <td className={`p-4 text-right font-mono font-bold ${s.netProfit >= 0 ? "text-emerald-600" : "text-destructive"}`}>{fmt(s.netProfit)}</td>
                          <td className={`p-4 text-right font-mono text-xs ${s.netMargin >= 0 ? "text-emerald-600" : "text-destructive"}`}>{s.netMargin.toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr className="border-t-2 bg-secondary/30 font-semibold">
                        ...
                        <td className={`p-4 text-right font-mono ${totals.grossProfit >= 0 ? "text-emerald-600" : "text-destructive"}`}>{fmt(totals.grossProfit)}</td>
                        <td className="p-4 text-right font-mono">{fmt(totals.expense)}</td>
                        <td className={`p-4 text-right font-mono ${totals.netProfit >= 0 ? "text-emerald-600" : "text-destructive"}`}>{fmt(totals.netProfit)}</td>
                        <td className={`p-4 text-right font-mono text-xs ${overallMargin >= 0 ? "text-emerald-600" : "text-destructive"}`}>{overallMargin.toFixed(1)}%</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
        ...
        <TabsContent value="soa" className="mt-0">
          <SOADashboard />
        </TabsContent>
      </Tabs>
    </DashboardLayout>
  ...
  return (
    <Card className={highlight ? "ring-2 ring-accent/30" : ""}>
      <CardContent className="p-4">
        ...
        <p className={`text-xl font-bold ${positive !== undefined ? (positive ? "text-emerald-600" : "text-destructive") : "text-foreground"}`}>{value}</p>
